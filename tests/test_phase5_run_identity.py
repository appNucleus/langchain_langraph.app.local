...
def test_legacy_thread_id_is_normalized_as_conversation_id() -> None:
    ...
    assert identity.execution_thread_id.startswith("conversation-123:")
...
def test_explicit_values_replace_defaults() -> None:
    ...
    request = ChatRequest(
        ...
        run_id=run_id,
        system_prompt="Use this exact prompt.",
        metadata={"client": "postman", "run_id": "must-not-win"},
    )

    identity = service.normalize(request)

    assert identity.conversation_id == "conversation-explicit"
    assert identity.run_id == run_id
    assert identity.client_supplied_run_id is True
...
def test_resume_token_is_bound_to_request_and_identity() -> None:
    settings = _settings()
    service = RunIdentityService(settings)
    original = ChatRequest(
        message="Continue the analysis",
        conversation_id="conversation-123",
        metadata={"scope": "phase-5"},
    )
    ...
    assert resumed.execution_thread_id == identity.execution_thread_id
